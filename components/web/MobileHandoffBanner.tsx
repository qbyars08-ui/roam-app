...
export default function MobileHandoffBanner({ hasGeneratedTrip }: MobileHandoffBannerProps) {
  ...
  const handleDismiss = useCallback(async () => {
    ...
    try { await AsyncStorage.setItem(DISMISS_KEY, String(Date.now())); } catch { /* noop */ }
  }, [slideAnim]);

  const handleGetApp = useCallback(() => { Linking.openURL(APP_STORE_URL); }, []);
      ...
      <View style={styles.inner}>
        ...
        <View style={styles.content}>
          ...
          <Text style={styles.body}>
            {t('web.handoffBody', {
              defaultValue: 'Get ROAM on your phone for live navigation, audio guides, and offline access.',
            })}
          </Text>
...
const styles = StyleSheet.create({
  container: {
    ...
    zIndex: 999, paddingHorizontal: SPACING.md, paddingBottom: SPACING.md,
  },
  inner: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.surface2,
    borderLeftWidth: MAGAZINE.accentBorder, borderLeftColor: COLORS.sage,
    borderRadius: RADIUS.md, paddingVertical: SPACING.md,
    paddingHorizontal: SPACING.md, gap: SPACING.sm,
  },
  content: { flex: 1 },
  heading: {
    fontFamily: FONTS.headerMedium, fontSize: 14, color: COLORS.cream, marginBottom: 2,
  },
  body: {
    fontFamily: FONTS.body, fontSize: 12, color: COLORS.creamDim,
    lineHeight: 18, marginBottom: SPACING.xs,
  },
  chips: { flexDirection: 'row', flexWrap: 'wrap', gap: SPACING.xs },
  chip: {
    backgroundColor: COLORS.sageVeryFaint, borderRadius: RADIUS.sm,
    paddingHorizontal: 6, paddingVertical: 2,
  },
  chipText: { fontFamily: FONTS.mono, fontSize: 10, color: COLORS.sage, letterSpacing: 0.3 },
  ctaButton: {
    backgroundColor: COLORS.sage, borderRadius: RADIUS.pill,
    paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm,
  },
  ctaText: { fontFamily: FONTS.bodyMedium, fontSize: 13, color: COLORS.bg },
