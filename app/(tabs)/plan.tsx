  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <ScrollView style={styles.fill} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <GoNowFeed />
        <Pressable onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); router.push('/(tabs)/flights' as never); }} accessibilityLabel="Search all flights" accessibilityRole="button" style={({ pressed }) => [styles.searchFlightsBtn, { opacity: pressed ? 0.85 : 1 }]}><Plane size={16} color={COLORS.bg} strokeWidth={1.5} /><Text style={styles.searchFlightsBtnText}>Search all flights</Text><ChevronRight size={16} color={COLORS.bg} strokeWidth={1.5} /></Pressable>
      </ScrollView>
