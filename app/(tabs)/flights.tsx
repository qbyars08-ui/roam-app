...
// deep links, best time to fly intel. No broken APIs, no fake data.
// =============================================================================
...
const BEST_TIME_TIPS = [
  ...
  { destination: 'Mexico City', months: 'Oct-Dec', tip: 'Day of the Dead through holidays. Fares spike in Dec.' },
  { destination: 'Bangkok', months: 'Nov-Feb', tip: 'Cool season. Book early Jan for the lowest prices.' },
...
function FlightsScreen() {
  ...
  const handleSearch = useCallback(() => {
    ...
    captureEvent('flight_search', { from: origin ?? 'unknown', to: dest });
  ...
  const handleRoutePress = useCallback((from: string, toCity: string) => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    captureEvent('flight_route_clicked', { from, to: toCity });
