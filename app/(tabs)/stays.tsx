// =============================================================================
// ROAM — Stays Tab
// Curated accommodation discovery with Booking.com affiliate links.
// Backpackers, couples, boutique sections. No broken APIs.
// =============================================================================
...
// ---------------------------------------------------------------------------
// Stay data — curated, always available
// ---------------------------------------------------------------------------
...
const CURATED_SECTIONS: {
  ...
  },
  {
    ...
    subtitle: 'Boutique hotels and charming stays',
    icon: Heart,
...
function StaysScreen() {
  ...
  const handleBookingSearch = useCallback((city: string) => {
    ...
    captureEvent('stays_booking_clicked', { destination: city });
    const url = getHotelLink({ destination: city });
    openBookingLink(url, 'booking', city, 'stays_tab').catch(() => {});
  }, []);

  const handleHeroSearch = useCallback(() => {
    const city = destination || 'anywhere';
    handleBookingSearch(city);
            ...
            <View key={section.key} style={styles.section}>
              ...
              {section.destinations.map((dest) => (
                <View key={dest.city} style={styles.destGroup}>
                  <Pressable
                    onPress={() => handleBookingSearch(dest.city)}
                    ...
                    >
                      <View style={styles.stayTop}>
                        <View style={styles.stayInfo}>
                          ...
                          <Text style={styles.stayNeighborhood}>{stay.neighborhood}</Text>
                        </View>
...
const styles = StyleSheet.create({
  ...
  heroBtnRow: { width: '100%', marginTop: SPACING.sm } as ViewStyle,
  heroBtn: {
