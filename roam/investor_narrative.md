...
**Layer 2 — People tab:** After you plan your trip, ROAM shows you who else is going there. AI-matched by destination, travel dates, and vibe (foodie, adventure, culture, etc.). Travelers see each other's match score, bio, vibe tags, and countries visited. One tap to connect. One tap to join a forming group trip.
...
- People tab is a local moat: German-speaking travelers matching with other German-speaking travelers creates a DACH-specific network that US-first apps cannot replicate
...
| Competitor | What They Built | Why They Failed | ROAM's Advantage |
|---|---|---|---|
| **Couchsurfing** | Hospitality + social layer for travelers | Went pay-to-play in 2020. Community revolted. Trust destroyed. Never recovered. | Free-first. Trust built through trip generation, not hosting strangers. |
| **Tourlina** | Solo female travel companion app | Hyper-niche (women only). No AI. No trip planning. No itinerary. Just a messaging app with travel theme. | Full-stack: planning + matching + prep. All genders. AI-powered vibe matching. |
| **Meetup** (travel groups) | Interest-based event groups | Not travel-specific. Not mobile-first. No AI. Group trips require a local organizer, not algorithmic matching. | Destination + dates + vibe = algorithmic match. No organizer required. |
| **Wanderlog** (groups) | Shared itinerary building | You share a trip with friends you already have. No way to discover new travel companions. | Discover travelers you don't know yet, matched by AI on destination + dates + vibe. |

**The gap in one sentence:** No product combines AI trip generation with traveler matching. You either get a planner (no social) or a social layer (no planning). ROAM is the first to do both.

**Market signal:** 80% of leisure trips involve more than one person. Yet zero scaled apps help you find that second person if you don't already know them. This problem is not small — it's just unsolved.
...
**Social layer:**
- Vibe-based matching — match score computed from travel style, destinations, dates, and pace preferences
- Trip → companion funnel — generate a trip, find people going to the same place; planning feeds social discovery
- Group trip formation — from individual match to group trip in two taps
- Network effects — more users = better matches = more value per user = harder to displace

## Business Model

### Revenue Streams
1. **Pro Subscription** ($9.99/month) — Unlimited trip generation, unlimited traveler matches, create group trips, direct messages, offline access
2. **Affiliate Commissions** — Skyscanner flights (~$0.50-2.00/click), Booking.com stays (~$2-5/booking)
3. **People Tab Pro Gating** — Free: see 3 matched travelers per destination, join 1 group. Pro: unlimited matches, create groups, message directly, see "who's there now"
4. **Premium Destinations** — Curated local experiences, restaurant partnerships (Phase 2)
...
| People Tab | Live — traveler matching, group trip cards, vibe-based match scores |
...
| DACH Strategy | Complete (10 scripts, platform research, creator outreach system) |
...
2. Fund DACH creator campaign ($5K for first 30 UGC videos — both planning and People tab hooks)
3. Cover 6 months of infrastructure scaling (Supabase Pro, Claude API burst capacity, People tab backend)
...
3. **Zero marginal data cost** — 10 free APIs provide real-time travel intelligence; competitors pay for this data
4. **AI-native from day one** — Not a travel site with AI bolted on; not a social app with planning bolted on; AI is woven through both layers
5. **UGC-first distribution** — "Find your travel squad on ROAM" is a TikTok video waiting to go viral; German TikTok travel is unclaimed territory
6. **EU founder advantage** — Austrian citizenship = EU entity, GDPR-native, DACH market access, German fluency
7. **Right market, right moment** — Germany is the #1 outbound travel market in Europe; Couchsurfing's collapse left a $B+ gap; no credible successor has emerged
...
Our beachhead: Germany. $70B outbound travel market. No dominant AI travel app. Couchsurfing's collapse left a massive gap in travel social. We're the first to combine both layers.

Pre-revenue. 5 tabs live. Raising $150-250K at a $2M cap.
